...
class BucketTests(unittest.TestCase):
    def test_every_prefix_including_empty_is_emitted(self):
        ...
        self.assertIn("CensusRun.Bucket255", buckets)
        self.assertIn("n : Nat := 0", buckets["CensusRun.Bucket1"])
        self.assertIn("inRange 255 8", buckets["CensusRun.Bucket255"])
    ...
    def test_assembly_has_no_packed_ids_and_uses_bucket_lemmas(self):
        ...
        self.assertEqual(source.count("BucketCertificates.cons"), 4)
        self.assertIn("CensusRun.Bucket3.equality", source)
    ...
    def test_prefix_knob_and_order_do_not_change_proofs(self):
        ...
        for bits in [0, 1, 4, 8]:
            ...
            self.assertEqual(len(first), 2 ** bits)
        for bits in [-1, 257]:
