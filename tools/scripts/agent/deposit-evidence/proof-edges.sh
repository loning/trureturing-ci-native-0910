...
# environment. usage: op-proof-edges.sh WORKTREE MODULE_DOTTED OUT_JSON(absolute)   sentinel: EDGES_OK n=<rows> | EDGES_FAIL
set -uo pipefail
...
json.dump(d,open(out,'w'),ensure_ascii=False,indent=1); print('EDGES_OK n=%d public=%d'%(len(edges),len(d['public_declarations'])))
PY
