...
2. **Escape content (CLAUDE.md, first-freeze shape and admission)**: for every public theorem check the PR body's `proof_shape` / direct frozen dependencies (GID + statement_id) / `escape_witness` (on the LIVE proof path, not smuggled) / module `admission_basis`. A first-freeze module with no admission basis is a reject.
