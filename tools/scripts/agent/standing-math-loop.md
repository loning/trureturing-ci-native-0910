...
### 这桩调查**已经关闭过一次**,别再开(第 7.11 条)

本会话之前已经把「怎么到 3 席」追到底并关闭了。结论与其算术照抄如下,
**下一个驱动者不必重新量,也不该重新猜**:

`make lean-report` 的调用链是
`lean-report.sh → tools/lean-inspector/inspect.sh → lake env lean --run Inspector.lean`,
**它是单个 `lean` 进程加载整个环境**(本树约 3,962 个 olean)做声明级检查。
**里面没有并行度可压** —— `serial-lean.sh` 那种「逐模块串行」的包装对它完全无效,
因为它本来就只有一个进程。

⟹ **每席的 `r_i` 有一个不可约下界 ≈ 4.2–4.4 GB。**
`C ≈ 14 GB`(16 GB 减系统与 orchestrator 常驻),`3 × 4.4 = 13.2` 只剩 0.8 GB 余量,
而**实测的每席真值是 5.2 GB**(`3 × 5.2 = 15.7 > 14`)。
同日按 `q=3` 派出的三席**在同一分钟内全灭**,与该算式互相印证。

**要到 3 席只有两条路,都不在会话内**:①内存 ≥ 24 GB;②改 inspector 的设计,
让它流式处理而不是把整个环境载进一个进程。二者皆 τ=0 决定(采购 / 器的重构),
**不是调度参数能解的**。按第 5.9 条记为具名 open。

**已经压下去的那一半要记清楚,免得重复劳动**:`make lean` 那一步是**真的**可压的
(lake 按核数无上限并行 → 曾观测 7 进程 14.25 GB),`serial-lean.sh`(#6648)把它压到单模块峰 3.38 GB。
**那一半已解决;剩下的一半在 inspector,不在调度。**

### 唯一还没测的那一问,以及它为什么低价值

没测的是:**席位改走 serial-lean 之后,整席峰值会不会从 5.2 GB 落到 4.4 GB 的地板附近?**
即便答案是「会」,`3 × 4.4 = 13.2` 对 `C ≈ 14` 只余 0.8 GB,
而**上一次按 14 GB 的估算派三席就是全灭的那一次** —— 峰值发生在派席之后,派席前的
`memory_pressure` 读数(当时 86% 空闲)不足以支持第三席。
故这一问即使测出好数,也**不足以改变 `q=2` 的结论**;真要改,先改①或②。

**写下这一段的理由**:我今天差点把它当成一条新预测重开一遍
(还预测「单席峰值 ≤ 4.0 GB」—— 那个数**低于已实测的 4.2–4.4 GB 不可约下界**,当场就是错的)。
第 7.11 条说同一症状第二次即停手去修根因;根因在权限外时,正解是停止投入 + 量化 + 立案 + 转向,
**不是换个说法再投一轮**。
...
# 追加(2026-09-10 会话)—— **选靶函数换了,以及两条把我驳倒的条文**

## 十五、发现三:**选靶该按「内容的性质」,不按「靶在哪个队列里」**

本会话五个实施靶的落地与否,按内容性质分得干干净净:

| 落地了 | 内容的性质 |
| --- | --- |
| `robin_seven_smooth` | **新的解析界** + 精确有理夹逼把无穷压成有限 |
| Gribinski `m=3` | **新的平方和证书**(20 个加权平方 + 767 个正单项) |
| Motzkin `Conjecture 7`(#6686) | **反驳**——新数值事实 `H₄ = −1` |

| 没落地 | 内容的性质 |
| --- | --- |
| 最小正修正(秩/迹) | **装配**:惯性变分刻画 + 迹范数三角不等式,两者**皆已冻结** |
| 5040 联合最优三角形 | **装配**:单调性已冻结,几何由钉版 Mathlib `AffineBasis` 一族给出 |

**根因**:仓内冻结集 + 钉版 Mathlib 已经厚到——源卷里「把已有零件接起来」型的定理,**大概率 bind-only**。
这不是靶不好,是**这个仓已经长到那一步了**。

**⟹ 派实施席之前问一句(写进 brief 顶部):**

> **这条定理的难点,是「没人算过的一个数 / 一条界」,还是「把已有引理接起来」?**

后者**不派**。前者才派。这一问比「它在不在 `deposit` 队列里」有用得多——
本会话两个不落地的靶,一个来自 B 档分诊、一个来自 `deposit` 队列,**队列身份没有预测力**。

**辨析(别把这条用过头)**:「装配」不等于「没价值」。它可以走
第 3.2 条的伴随声明、`make cover`、或作为别的模块的私有引理;**只是不单独开实施席、不单独冻**。

## 十六、**普通正向有限实例这条路是封死的**(选靶时先排除,别到写完才发现)

第 3.3 条:`certified-instance` 与 `bounded-enumeration` **只可走经验证的 `refutes`**;
consumer / terminal / 自指终点 / 附加 `result` **一律不豁免**。

所以下列形态**在选靶阶段就该排除**,不要等到证完才发现进不了门:

- 「算出某个具体 `n` 的证书」(哪怕带严格有理区间);
- 「枚举到 `N` 显示某性质」;
- 「验证第 `k` 行/第 `k` 个情形」。

**唯一还开着的口**是:**该有限计算处决一条具名断言**。
本会话 #6686 走的正是这条——`H₄ = −1` 处决 arXiv:2502.21050v1 的 `Conjecture 7` 印刷全称形。
写这类模块时:`claim` 必须是**印刷/原始文本的全称形**(只写特例就是在反驳自己造的命题),
`result` 是它的否定,有限枚举**全部 private**。

**具体判例**:z5040 卷 `定理 11.2`(5040 单元的严格 Robin 证书,`Δ(5040) < 0` 加单元内下界)
**看起来**极好派——仓内 `D5/S3/Arith/GoldenResource/RobinRationalBasis` 已冻结,
带 `robinDelta`、`RationalBracket`、`RobinPositiveJudge` + soundness、
`log_two_decimal_bounds`、`eulerMascheroni_decimal_bounds`、`rational_log_bounds`,还有 10080 的实例。
**但它是普通正向有限实例**,按本节封死。**2026-09-10 因此判不派。**

## 十七、**spec A17.2:第三方 Lean 只有依赖与移植两种准入形**(我在这条上被驳倒)

`docs/develop/spec/golden-ledger-repo-spec.md` A17.2 逐字:

> 精确命中的第三方 Lean 成果有且仅有两种合法准入形——**依赖**与**移植**;**重证禁止**。
> 立本条之由:第 11 条有序路径的 ③ 指向「按 spec A17 可准入的第三方 Lean 生态」,
> 而 A17 的三条准入谓词现役全为 `open`,故 ③ 事实上关闭;
> **若不另立移植形,路径塌缩为 ②→④本地证明,恰是同条所禁之重证。**

**它把「上游不可依赖又不可移植,所以我们本地证」这条塌缩逐字点名并禁止了。**
我 2026-09-10 正是这么推的(据 CLAUDE.md 第 3.1 条有序路线),**被评审席引本条驳回**。

**形式选择由机器判,不由偏好判**:比上游 `lean-toolchain` 与其 `lake-manifest.json` 里的
mathlib `rev` 与本仓同名值——**不等即依赖形不可行,无须再议;不等则只余移植形或放弃**。
移植形四项义务:①许可证与 NOTICE 整链带入 ②入仓后照常执法不豁免
③axiom/sorry 闭包只含标准三条 ④**退役条件须对本仓自己的钉版可判**
(禁止以「上游被 mathlib 接受」为到期条件,A17.2 自带反例)。

**教训的形状**:我读了 CLAUDE.md 正文,**没读 spec 的续条**。
这是记忆里那条「spec 紧邻条款先读」的第二次发作。**引 CLAUDE.md 条款做承重判断前,先 grep 一遍 spec 的对应节。**

## 十八、**争「支配 / 见证」之前,先跑那个 26 秒的探针**

同一个靶上,实施席以「上游 `moser` 有支配定理」撤回几何见证,我以「那个上游不可用故应算新内容」质疑。
**两条都没打中。** 评审席写了 `MathlibBindOnly.lean`,`lake env lean` **EXIT=0、26.4 秒**,
用钉版 Mathlib 的一条具名链把两条几何陈述都证了:
...
⟹ 该步 bind-only,**上游存不存在都不影响**。

**⟹ 「某步骤算不算逃逸见证」有一个可判的形式,先跑它,再谈政策:**
写 run-local 探针,只用**钉版 Mathlib + 冻结件 + 规范化改写**证同一陈述;跑通即 bind-only。
凸包 / 仿射无关 / 重心坐标这类,Mathlib 有整套 `AffineBasis` 机器,**别因为找不到同名引理就判它没有**。

## 十九、本会话新增的三条现成资产(接手可直接用)

1. **B 档 41 条的逐条缺口**已贴在 #6160 第 66 轮(此前只有计数,逐条清单曾只存在于未合分支
   `lane/math/tier3-mathlib-triage-0909` 的 `e12ff7dd35`)。
   **但重筛结论是**:剩余 36 条里 `unresolvable-without-object` **21** / `still-open` **14** / `repo-closed` **1**
   (那 1 条还是种子本身)⟹ **B 档作为靶源已耗尽,别再从那里选。**
2. **`readiness` 读数**(dev `6428a0eb90`):`deposit` 靶 **508**,其中 `quantum-rh` **60**、
   `zeckendorf-euler-5040` **74**;全库 `not-formalizable` **18,322**。已发 #5952。
3. **#6494 第 5 轮**给了两条第三档地形图:`2606.21407v2` Conjecture 7.2(Grivaux,结果式 Hurwitz 稳定性)、
   `1308.3678v3` Question 2.7(2)(相邻 CA 数之商能否为半素数)。后者与 z5040 卷 `定理3.1`
   「不存在三个不同素数同时临界」是同一战场——**仓内有三素数结果,开放的是两素数版**;
   但该定理用六指数定理(Pila),**Mathlib 无**,形式化须记 `AxiomDebt` 或写成条件形。

## 二十、边界(不冒领)

- 十五节的「大概率」是**对五个样本的读判**,**不是分布测量**,不据此声称任何比例;
- 十六、十七两节引的是条文原文,**判据是条文,不是我的转述**——动手前自己 grep 一遍;
- 十八节那条具名链**我没有自己跑过**,是评审席的读数(它给了 `EXIT`、公理与路径);
- 本文件仍**没有 fail-closed 机器消费者**,不保证被执行;真正的驱动仍是 owner 侧的定时 prompt。
