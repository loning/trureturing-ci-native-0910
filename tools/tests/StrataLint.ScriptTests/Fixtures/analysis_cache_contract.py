...
# Keep the real make recipe and shell entrypoint. Only replace dotnet run's build
# phase with the candidate DLL already compiled by this test project.
DOTNET_LAUNCHER = '''#!/usr/bin/env bash
set -euo pipefail
[[ "$#" -eq 8 && "$1" == run && "$2" == --project &&
   "$3" == "$PWD/tools/StrataLint.Cli/StrataLint.Cli.csproj" &&
   "$4" == --configuration && "$5" == Release && "$6" == -- &&
   "$7" == worktree && "$8" == ensure-cache ]] || exit 86
