...
public sealed class UtilityDepositEntryTests
{
    ...
    public void CanonicalMakeDepositUsesRealReportAndBaselineForUtilityPrecheck(bool refutation)
    {
        ...
        File.WriteAllBytes(obligations, Run(cli, ["lean-utility-input"], root).StandardOutput);
        var report = Path.Combine(root, ".lake", "build", "stratalint", "raw-lean-report.json");
        Directory.CreateDirectory(Path.GetDirectoryName(report)!);
        Run("lake", ["env", "lean", "--run", Path.Combine(repository, "tools/lean-inspector/Inspector.lean"),
        ...
        Run("python3", [Path.Combine(repository, "tools/lean-inspector/materials.py"), "compact",
        ...
        WriteExecutable(root, "dotnet", """
            ...
            """);
        var baseline = Encoding.UTF8.GetString(Run("git", ["rev-parse", "HEAD"], root).StandardOutput).Trim();
    ...
    private static ProcessOutput Run(string executable, string[] arguments, string root)
    {
