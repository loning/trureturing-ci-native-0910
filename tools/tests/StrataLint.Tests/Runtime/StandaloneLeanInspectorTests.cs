...
public sealed class StandaloneLeanInspectorTests
{
    ...
    [Fact]
    public void EncoderEmitsSharedExpressionsWithoutMaterialSizedFragments()
    {
        using var temporary = new TemporaryDirectory();
        RunEncoderProbe(temporary.Path, """
            #eval show IO Unit from do
              ...
              let largest ← IO.mkRef 0
              let status ← try
                emitStatement (fun fragment => do
                  if fragment.utf8ByteSize > 64 then throw <| IO.userError "material-sized fragment"
                  bytes.modify (· + fragment.utf8ByteSize)
                  calls.modify (· + 1)
                  largest.modify (max · fragment.utf8ByteSize)) info
                pure "complete"
              catch error => pure error.toString
              IO.FS.writeFile "observation" s!"{status}\n{← bytes.get}\n{← calls.get}\n{← largest.get}"
            """);
        var observed = File.ReadAllLines(Path.Combine(temporary.Path, "observation"));
        Assert.Equal("complete", observed[0]);
        // Each app adds five framing bytes around two shared children.
        var expandedBytes = (14 + 5) * (1 << 16) - 5;
        Assert.Equal(expandedBytes + Encoding.UTF8.GetByteCount("statement-v1(uparams=[],type=)"), int.Parse(observed[1]));
        Assert.True(int.Parse(observed[2]) > 1 << 16);
        Assert.InRange(int.Parse(observed[3]), 1, 64);
    }

    [Fact]
    public void EncoderStopsAtSynchronousSinkFailure()
    {
        using var temporary = new TemporaryDirectory();
        RunEncoderProbe(temporary.Path, """
            #eval show IO Unit from do
              let mut expression := Expr.bvar 1
              for _ in [:16] do expression := .app expression expression
              let info : ConstantInfo := .axiomInfo {
                name := .anonymous, levelParams := [], type := expression, isUnsafe := false }
              let calls ← IO.mkRef 0
              let status ← try
                emitStatement (fun _ => do
                  calls.modify (· + 1)
                  if (← calls.get) == 7 then throw <| IO.userError "injected sink failure") info
                pure "completed without error"
              catch error => pure error.toString
              IO.FS.writeFile "observation" s!"{status}\n{← calls.get}"
            """);
        Assert.Equal(["injected sink failure", "7"], File.ReadAllLines(Path.Combine(temporary.Path, "observation")));
    }

    [Fact]
    public void EncoderEmitsLiteralPayloadSeparatelyFromItsFraming()
    {
        using var temporary = new TemporaryDirectory();
        RunEncoderProbe(temporary.Path, """
            #eval show IO Unit from do
              ...
              let payloads ← IO.mkRef 0
              let bytes ← IO.mkRef 0
              let status ← try
                emitStatement (fun fragment => do
                  if fragment.utf8ByteSize > payload.utf8ByteSize then
                    throw <| IO.userError "wrapped literal"
                  if fragment == payload then payloads.modify (· + 1)
                  bytes.modify (· + fragment.utf8ByteSize)) info
                pure "complete"
              catch error => pure error.toString
              IO.FS.writeFile "observation" s!"{status}\n{← payloads.get}\n{← bytes.get}"
            """);
        ...
        Assert.Equal(131072 + Encoding.UTF8.GetByteCount("statement-v1(uparams=[],type=ei(lt(131072:)))"), int.Parse(observed[2]));
    }
