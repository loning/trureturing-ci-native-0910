...
internal sealed class MultiplicativeExclusionResidueClassDocument : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Paragraph(Text(
                "The rule is stated with the atom's integer subtraction a(i)a(j) \u2212 a(k); "
                    + "Theorem literal_exclusion_iff_additive records the subtraction-free form "
                    + "used in the proofs. At prospective index n, a'(r) is x when r=n and is "
                    + "the prior sequence value a(r) otherwise. Thus k=n, including the "
                    + "self-witness 2+2=2*2, is retained literally.")),
            Describe.Lean(
                DescribeId.Create("multiplicative-exclusion-literal-witness-set"),
                DeclarationHandle.Create(Prefix + "S"),
                H("Literal witness set"),
                StatementSource.FromAuthor(WitnessSetFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "This is the atom's witness set verbatim: the exceptional element 3 "
                        + "together with every natural number of the form 3t+1."))),
                DescribeRole.Definition),
            Describe.Lean(
                DescribeId.Create("multiplicative-exclusion-witness-set-residue-identity"),
                DeclarationHandle.Create(Prefix + "mem_S_iff"),
                H("Literal witness set equals the residue description"),
                StatementSource.FromAuthor(WitnessSetIdentityFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "Division with remainder identifies the progression 3t+1 with residue "
                        + "one modulo three, while preserving the exceptional element 3."))),
            ...
            Describe.Lean(
                ...
                DescribeRole.Definition),
            Describe.Lean(
                DescribeId.Create("multiplicative-exclusion-literal-additive-identity"),
                DeclarationHandle.Create(Prefix + "literal_exclusion_iff_additive"),
                H("Integer subtraction is equivalent to the additive exclusion test"),
                StatementSource.FromAuthor(LiteralExclusionIdentityFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "For natural x,a,b,c, the left inequality is interpreted in the integers. "
                        + "Moving c across the equality yields exactly the natural-number form "
                        + "used by the internal exclusion predicate."))),
                DescribeRole.Theorem),
            Describe.Lean(
                DescribeId.Create("multiplicative-exclusion-rule-additive-bridge"),
                DeclarationHandle.Create(Prefix + "satisfiesLiteralRule_iff_additive"),
                H("The literal and additive sequence rules are identical"),
                StatementSource.FromAuthor(SatisfiesAdditiveBridgeFormula()),
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "The identity is applied pointwise to every prospective candidate. This "
                        + "public bridge leaves the covered theorems stated for the literal rule "
                        + "while the proof uses addition in the natural numbers."))),
                DescribeRole.Theorem),
            Describe.Lean(
                ...
                AssessedProvenance.FromRepo(),
                Blocks(Paragraph(Text(
                    "If x>=4 and x,u,v,w lie in the literal set S "
    ...
    private static Formula WitnessSetFormula()
    {
        Formula t = F.Id("t");
        Formula progression = new Formula.SetBuilder(
            Add(Seq(D(3), t), D(1)), t, Naturals());
        return Disp(Seq(
            F.Id("S"), Sp, Eq, Sp, new Formula.SetLiteral([D(3)]),
            Sp, Cup, Sp, progression, Dot));
    }

    private static Formula WitnessSetIdentityFormula()
    {
        Formula x = F.Id("x");
        return Disp(Seq(
            Forall, Sp, x, Sp, InMacro, Sp, Naturals(), Comma, Sp,
            Membership(x), Sp, Iff, Sp, Parenthesized(Protected(x)), Dot));
    }
    ...
    private static Formula LiteralExclusionIdentityFormula()
    {
        Formula x = F.Id("x");
        Formula a = F.Id("a");
        Formula b = F.Id("b");
        Formula c = F.Id("c");
        Formula literal = NotEqual(
            IntegerCast(x),
            Subtract(Multiply(IntegerCast(a), IntegerCast(b)), IntegerCast(c)));
        Formula additive = NotEqual(Add(x, c), Multiply(a, b));

        return Disp(Seq(
            Forall, Sp, x, Comma, Sp, a, Comma, Sp, b, Comma, Sp, c,
            Sp, InMacro, Sp, Naturals(), Comma, Sp,
            Parenthesized(literal), Sp, Iff, Sp, Parenthesized(additive), Dot));
    }

    private static Formula SatisfiesAdditiveBridgeFormula()
    {
        Formula a = F.Id("a");
        Formula n = F.Id("n");
        Formula additiveBody = Conjunction(
            Equal(At(a, D(0)), D(0)),
            Equal(At(a, D(1)), D(1)),
            ForAll(n, Implies(LessOrEqual(D(2), n), AdditiveLeastClause(a, n))));

        return Disp(Seq(
            Forall, Sp, a, Colon, Sp, FunctionType(Naturals(), Naturals()), Comma, Sp,
            Call("SatisfiesLiteralRule", a), Sp, Iff, Sp,
            Parenthesized(additiveBody), Dot));
    }
    ...
    private static Formula ResidueProtectionFormula()
    {
        ...
        Formula assumptions = Conjunction(
            LessOrEqual(D(4), x),
            Parenthesized(Membership(x)), Parenthesized(Membership(u)),
            Parenthesized(Membership(v)), Parenthesized(Membership(w)),
    ...
    private static Formula WitnessFormula()
    {
        ...
        Formula witnessBody = Conjunction(
            Parenthesized(Membership(u)), Parenthesized(Membership(v)),
            Parenthesized(Membership(w)),
        ...
        return Disp(new Formula.Aligned([
            ...
            Seq(Parenthesized(Not(Membership(y))), Sp, Rightarrow),
            Seq(witness, Dot),
        ]));
    }
    ...
    private static Formula SequenceDefinitionFormula()
    {
        ...
        Formula exists = ExistsOne(x, AdditiveAdmissible(history, next, x));
    ...
    private static Formula SequenceRecurrenceFormula()
    {
        ...
        return Disp(new Formula.Aligned([
            ...
            Seq(LiteralLeastClause(F.Id("sequence"), n), Dot),
        ]));
    }
    ...
    private static Formula LiteralLeastClause(Formula a, Formula n)
    {
        Formula x = F.Id("x");
        Formula value = At(a, n);
        Formula membership = Parenthesized(LiteralAdmissible(a, n, value));
        Formula minimality = Parenthesized(ForAll(x,
            Implies(LiteralAdmissible(a, n, x), LessOrEqual(value, x))));
        return Conjunction(membership, minimality);
    }

    private static Formula AdditiveLeastClause(Formula a, Formula n)
    {
        Formula x = F.Id("x");
        Formula value = At(a, n);
        Formula membership = Parenthesized(AdditiveAdmissible(a, n, value));
        Formula minimality = Parenthesized(ForAll(x,
            Implies(AdditiveAdmissible(a, n, x), LessOrEqual(value, x))));
        return Conjunction(membership, minimality);
    }

    private static Formula LiteralAdmissible(Formula a, Formula n, Formula x)
    {
        Formula i = F.Id("i");
        Formula j = F.Id("j");
        Formula k = F.Id("k");
        ...
        Formula exclusion = NotEqual(
            ...
            Subtract(
                Multiply(
                    IntegerCast(Updated(a, n, x, i)),
                    IntegerCast(Updated(a, n, x, j))),
                IntegerCast(Updated(a, n, x, k))));
    ...
    private static Formula AdditiveAdmissible(Formula a, Formula n, Formula x)
    {
        Formula i = F.Id("i");
        Formula j = F.Id("j");
        Formula k = F.Id("k");
    ...
    private static Formula Membership(Formula value) =>
        Seq(value, Sp, InMacro, Sp, F.Id("S"));
