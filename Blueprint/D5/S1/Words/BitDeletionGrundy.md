...
**Definition 1.1 (The bounded mex scanner).**

$$(\forall S: \operatorname{Finset}(\mathbb{N}), \forall candidate: \mathbb{N}, \operatorname{mexScan}(S, 0, candidate) = candidate) \land (\forall S: \operatorname{Finset}(\mathbb{N}), \forall fuel: \mathbb{N}, \forall candidate: \mathbb{N}, \operatorname{mexScan}(S, fuel + 1, candidate) = \operatorname{ite}((candidate \in S), \operatorname{mexScan}(S, fuel, candidate + 1), candidate))$$

*Formalization.* `D5/S1/Words/BitDeletionGrundy.mexScan` (`✓ std3`).

*Source.* Repository-derived.

*Commentary.*

At zero fuel the scanner returns its candidate. With successor fuel, it increments a used candidate and recurses, or returns an unused one.

**Definition 1.2 (Minimum excluded value).**
...
**Theorem 1.3 (The bounded scan is mex).**
...
**Definition 1.4 (Leading-zero normalization).**
...
**Definition 1.5 (Positional one-bit erasures).**
...
**Definition 1.6 (Bit deletion on binary words).**
...
**Definition 1.7 (The even-suffix transition).**
...
The public witness map T_0 sends state value 1 to 3 and every other value in Fin 4 to 1.

**Definition 1.8 (The odd-suffix transition).**
...
The public witness map T_1 sends state value 0 to 2 and every other value in Fin 4 to 0.

**Definition 1.9 (Word-length parity).**
...
**Definition 1.10 (The parity-indexed transition).**
...
**Definition 1.11 (The closed-form automaton).**
...
**Theorem 1.12 (Appending two zeros preserves the formula).**
...
**Theorem 1.13 (The automaton computes the word game).**
...
For n in N, remove each positional digit from Mathlib's little-endian Nat.digits 2 n and re-encode the remaining list with Nat.ofDigits 2. The result is a Finset, so duplicate numerical outcomes are identified. Nat.ofDigits drops any high zero digits automatically; this is exactly the atom's leading-zero normalization. The atom states a natural-number recurrence, and this module's word model corresponds to it under binary decoding.
...
*Proof.* Machine-checked in Lean as `D5/S1/Words/BitDeletionGrundy.g_eq_wordGrundy` (`✓ std3`). ∎
...
The digit-decoding correspondence identifies every normalized word deletion with exactly one Nat.ofDigits successor, and identifies its wordGrundy value with g. Rewriting the existing word mex equation therefore gives the atom's public natural-number recurrence, to which this module's word model corresponds under binary decoding.
...
This is the whole preregistered candidate theorem: all values are at most three, and g(4n)=g(n) for every natural n.
