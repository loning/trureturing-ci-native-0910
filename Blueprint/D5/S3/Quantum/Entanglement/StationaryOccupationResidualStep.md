...
$$\forall A \in Type,\; \left(Fintype\left(A\right) \land \left(DecidableEq\left(A\right) \land Nonempty\left(A\right)\right)\right) \Rightarrow \left(\forall a \in Multiset\left(A\right),\; PositiveResidualStep\left(a\right)\right)$$

*Proof.* Machine-checked in Lean as `D5/S3/Quantum/Entanglement/StationaryOccupationResidualStep.positive_residual_step` (`✓ std3`). ∎
...
*Proof.* Machine-checked in Lean as `D5/S3/Quantum/Entanglement/StationaryOccupationResidualStep.stationary_memory_dimension_attained` (`✓ std3`). ∎
...
The residual transitions imply every word coefficient by induction. The normalized sector and the unitary circuit imply that the chosen initial residual has norm one. The common final memory is the sink basis vector. This includes zero occupation and letters of zero capacity; all physical memory is Fin(d(a)).

## References

- Truth anchor: `D5/S3/Quantum/Entanglement/StationaryOccupationResidualStep.positive_residual_step`
- Truth anchor: `D5/S3/Quantum/Entanglement/StationaryOccupationResidualStep.stationary_memory_dimension_attained`
- Dependency: [D5/S3/Quantum/Entanglement/StationaryOccupationResidualCircuit](StationaryOccupationResidualCircuit.md)
