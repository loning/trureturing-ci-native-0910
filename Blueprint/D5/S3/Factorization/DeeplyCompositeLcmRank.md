...
**Definition 1.1 (The first differing positive divisor favors the left number).**

$$\forall n \in \mathbb{N}, m \in \mathbb{N},\; DivPlusPrecedes\left(n, m\right) \Leftrightarrow \left(\exists d \in \mathbb{N},\; 1 \le d \land \left(d \mid n \land \left(\left(\neg (d \mid m)\right) \land \left(\forall e \in \mathbb{N},\; 1 \le e \Rightarrow \left(e < d \Rightarrow \left(e \mid n \Leftrightarrow e \mid m\right)\right)\right)\right)\right)\right)$$

*Formalization.* `D5/S3/Factorization/DeeplyCompositeLcmRank.DivPlusPrecedes` (`✓ std3`).

*Source.* Repository-derived.

*Commentary.*

This repository definition says that some positive d divides n but not m, and divisibility by every smaller positive e agrees for n and m. Choosing the least differing divisor is equivalent to lexicographically ordering the Div+ divisor vectors from OEIS A095848 (Switkay, comment 2023-08-27).

**Definition 1.2 (Deeply composite numbers are positive left-to-right minima).**

$$\forall n \in \mathbb{N},\; DC\left(n\right) \Leftrightarrow \left(1 \le n \land \left(\forall m \in \mathbb{N},\; 1 \le m \Rightarrow \left(m < n \Rightarrow DivPlusPrecedes\left(n, m\right)\right)\right)\right)$$

*Formalization.* `D5/S3/Factorization/DeeplyCompositeLcmRank.DC` (`✓ std3`).

*Source.* Repository-derived.

*Commentary.*

DC(n) means that n is positive and DivPlusPrecedes(n,m) holds for every positive m below n. This is the repository encoding used by DeeplyCompositeNotPrimeExponentRecord.

**Definition 1.3 (Rank counts deeply composite numbers through n).**
...
**Theorem 1.8 (A first-difference witness is bounded by the left number).**

$$\forall n \in \mathbb{N}, m \in \mathbb{N},\; 1 \le n \Rightarrow \left(DivPlusPrecedes\left(n, m\right) \Leftrightarrow \left(\exists d \in \mathbb{N},\; 1 \le d \land \left(d \le n \land \left(d \mid n \land \left(\left(\neg (d \mid m)\right) \land \left(\forall e \in \mathbb{N},\; 1 \le e \Rightarrow \left(e < d \Rightarrow \left(e \mid n \Leftrightarrow e \mid m\right)\right)\right)\right)\right)\right)\right)\right)$$

*Proof.* Machine-checked in Lean as `D5/S3/Factorization/DeeplyCompositeLcmRank.divPrecedes_iff_first_difference` (`✓ std3`). ∎

*Source.* Repository-derived.

*Commentary.*

For positive n, DivPlusPrecedes(n,m) is equivalent to the same defining witness with the additional bound d <= n. The bound follows because a positive divisor of n cannot exceed n. This is a repository lemma.

**Theorem 1.9 (The universal record condition is a finite filter identity).**

$$\forall n \in \mathbb{N},\; DC\left(n\right) \Leftrightarrow \left(1 \le n \land filter\left(Ico\left(1, n\right), DivPlusPrecedes\left(n\right)\right) = Ico\left(1, n\right)\right)$$

*Proof.* Machine-checked in Lean as `D5/S3/Factorization/DeeplyCompositeLcmRank.dc_iff_left_to_right_minimum` (`✓ std3`). ∎

*Source.* Repository-derived.

*Commentary.*

DC(n) is equivalent to positivity together with the statement that filtering every positive predecessor by DivPlusPrecedes(n,-) removes nothing. This finite-set identity is a repository characterization; the public Nat.nth bridge below supplies the sequence-indexed form.

**Theorem 1.10 (The next lcm prefix is bounded by endpoint multiplication).**
...
**Theorem 1.11 (Every reached lcm prefix divides a deeply composite number).**
...
**Theorem 1.12 (Few deeply composite records lie below an lcm prefix).**
...
**Theorem 1.13 (A deeply composite number has positive rank).**
...
**Theorem 1.14 (The square-root-rank lcm divides every deeply composite number).**
...
**Theorem 1.15 (Every enumerated term is deeply composite).**
...
**Theorem 1.16 (Enumeration index and deeply composite rank agree).**
...
**Theorem 1.17 (The square-root-rank lcm law for the enumerated sequence).**
...
- Truth anchor: `D5/S3/Factorization/DeeplyCompositeLcmRank.DC`
- Truth anchor: `D5/S3/Factorization/DeeplyCompositeLcmRank.DivPlusPrecedes`
...
- Truth anchor: `D5/S3/Factorization/DeeplyCompositeLcmRank.dc_iff_left_to_right_minimum`
...
- Truth anchor: `D5/S3/Factorization/DeeplyCompositeLcmRank.divPrecedes_iff_first_difference`
