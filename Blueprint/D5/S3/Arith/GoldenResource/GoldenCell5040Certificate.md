...
$$\forall L \in \mathbb{N},\; wValue\left(L\right) = fib\left(L + 2\right)$$

*Formalization.* `D5/S0/Conventions/WDigits.wValue` (`✓ std3`).

*Source.* Repository-derived.

*Commentary.*

For natural L, the canonical wValue L is exactly Fib(L+2), the weight G_L in ZECKENDORF_EULER_5040.
...
robinLogMargin is the total function on the naturals given by Euler's constant plus log(log(log n)) minus log(sigma_1(n)/n). The source states its margin only for n at least three; this definition agrees with it there and extends it by the same expression elsewhere, and every theorem below applies it only at cell members, each of which exceeds five thousand.
...
Every n in goldenCellPlus 5040 has robinLogMargin n greater than 1/100, which implies theorem 11.2's lower bound of 0.0047.

**Theorem 1.25 (Strict Robin certificate for the 5040 cell).**

$$robinLogMargin\left(5040\right) < 0 \land \left(\forall n \in \mathbb{N},\; n \in goldenCell\left(5040\right) \setminus \{5040\} \Rightarrow robinLogMargin\left(n\right) > 0.0047\right)$$
...
This is exactly the two boxed clauses of ZECKENDORF_EULER_5040 theorem 11.2: the margin at 5040 is negative, while every other point of its golden cell has margin greater than 0.0047. The proof is unconditional.
...
- Truth anchor: `D5/S0/Conventions/WDigits.wValue`
