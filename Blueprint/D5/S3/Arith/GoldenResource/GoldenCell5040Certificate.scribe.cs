...
internal sealed class GoldenCell5040CertificateDocument : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            Entry("wValue", "golden-cell-weight", "Golden Fibonacci weights",
                WValueFormula(), DescribeRole.Definition,
                "For natural L, the canonical wValue L is exactly Fib(L+2), the weight G_L in "
                    + "ZECKENDORF_EULER_5040.",
                "D5/S0/Conventions/WDigits.wValue"),
            ...
            Entry("robinLogMargin", "golden-cell-robin-log-margin",
                ...
                DescribeRole.Definition,
                "robinLogMargin is the total function on the naturals given by Euler's constant plus "
                    + "log(log(log n)) minus log(sigma_1(n)/n). The source states its margin only "
                    + "for n at least three; this definition agrees with it there and extends it by "
                    + "the same expression elsewhere, and every theorem below applies it only at "
                    + "cell members, each of which exceeds five thousand."),
            ...
            Entry("robin_log_margin_5040_cell_plus_gt", "golden-cell-5040-strong-margin",
                ...
                "Every n in goldenCellPlus 5040 has robinLogMargin n greater than 1/100, "
                    + "which implies theorem 11.2's lower bound of 0.0047."),
            Entry("robin_log_margin_5040_cell_certificate", "golden-cell-5040-certificate",
                ...
                "This is exactly the two boxed clauses of ZECKENDORF_EULER_5040 theorem 11.2: "
                    ...
                    + "has margin greater than 0.0047. The proof is unconditional."),
    ...
    private static DocumentBlock.Describe Entry(string declaration, string id, string title,
        Formula formula, DescribeRole role, string text, string? declarationHandle = null) =>
        Describe.Lean(
            DescribeId.Create(id),
            DeclarationHandle.Create(declarationHandle ?? Prefix + declaration), H(title),
            StatementSource.FromAuthor(formula), AssessedProvenance.FromRepo(),
            Blocks(Paragraph(Text(text))), role);

    private static Formula WValueFormula()
    {
        Formula ell = F.Id("L");
        return Disp(ForAll([Bound("L", Naturals())],
            Equal(Call("wValue", ell), Call("fib", Add(ell, Num(2))))));
    }
    ...
    private static Formula CertificateFormula()
    {
        ...
        Formula positive = ForAll([Bound("n", Naturals())],
            Implies(Member(n, cellDifference), Gt(Margin(n), Decimal0047())));
    ...
    private static Formula Gt(Formula left, Formula right) =>
        new Formula.Relation(left, FormulaRelationOperator.GreaterThan, right);
    private static Formula Decimal0047() => Seq(Num(0), Dot, D(0, 0, 4, 7));
