...
internal sealed class DeeplyCompositeLcmRankDocument : IScribeDocumentDefinition
{
    ...
    public DocumentDefinition Create() => DocumentDefinition.Create(ScribeNode.Create(
        ...
        Blocks(
            ...
            Theorem(
                ...
                "Rank equals the source's literal count through n",
    ...
    private static Formula RankEqCardFilterLeFormula()
    {
        Formula n = F.Id("n");
        Formula m = F.Id("m");
        Formula literalCount = Seq(
            OpenBrace, m, Sp, InMacro, Sp, Naturals(), Sp, Mid, Sp,
            Parenthesized(And(LessEqual(m, n), Call("DC", m))), CloseBrace);
        return ForAll(
            [Bound("n", Naturals())],
            Equal(Call("rank", n), Call("card", literalCount)));
    }
