...
*Proof.* Machine-checked in Lean as `D5/S3/Arith/RationalCompositionParityPeriodTen.triple_B_eq_formula_two` (`✓ std3`). ∎

*Source.* Repository-derived.

*Commentary.*

Proof: the proof establishes the required nonzero denominator facts before field normalization. This unconditional identity is in Q(x), so no pointwise non-pole hypotheses occur. Substituting Bf(y)=y/(1-y)^2 through three iterations yields the factored numerator and squared quartic denominator of formula (2).
...
- Truth anchor: `D5/S3/Arith/RationalCompositionParityPeriodTen.triple_B_eq_formula_two`
