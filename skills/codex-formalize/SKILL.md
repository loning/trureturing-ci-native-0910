...
Apply `CLAUDE.md`'s "library before proof" section. Search pinned mathlib and the repository's `D5/` declarations for the complete statement and for lemmas that close its dependencies. Record every query verbatim and record whether it hit.
...
make preflight BASE="$(git rev-parse HEAD^1)"   # early feedback only; NOT a gate (CLAUDE.md, per-layer tool entrypoints)
```
...
`make deposit` and `make cover` must exit 0. `make preflight BASE="$(git rev-parse HEAD^1)"` is local early feedback: report its exit code and every rejected rule verbatim, but a nonzero local preflight does not stop the lane — the three remote required checks are the only judgment (CLAUDE.md, per-layer tool entrypoints; push and local verification run in parallel). This macOS host has known preflight noise (Perl `C.UTF-8` locale crashes in script tests, `ENGINEERING_TEST_EVIDENCE_FAILED` without TRX, an `SL-022` observation line); stop only on a content-level red you caused (Lean error, `SL-008` frozen-surface diff, `SL-010` generality, digestion coverage), and then end as `open`. Judge them only by exit code, and never pipe a judgment command: `cmd | tail -1` reports the pipe's exit status, not the command's, and three landed incidents (a merge that silently failed, validation run on a stale base, a cover failure read as success) trace to exactly this. Run the command bare, or capture `$?` on the command itself before any formatting.
...
- **`NOT_COMPLETE` is a runner-side verdict about artifacts, not a statement about the work.** Before
  ...
  envelope is recoverable. This is the codex-log rule (CLAUDE.md, job lifecycle and logs) applied to the batch runner: the
...
- Never deposit a first-freeze module without an `admission_basis` (`escape-witness` | `rule-11-upstream-wrapper` | `atom-required-bridge`); `CLAUDE.md`'s first-freeze admission section owns this rule (report `open` with `proof_shape: bind-only` instead).
- Never invent a "needs human review" outcome; `CLAUDE.md`'s permanent prohibition forbids human-review gates outright.
