...
/-- The disk conclusion is transported to the actual open right half-plane,
with the inverse Mobius coordinate proved to lie in the disk. -/
theorem canonical_li_summable_right_half_plane
  ...
  have nonzero := (canonical_li_summable_disk_zero_free summable).2.2 z inDisk
...
theorem canonical_li_disk_summability_implies_rh
  ...
  intro s zero hhalf hone
  have xiZero : xiReading s = 0 :=
    (xiReading_eq_zero_iff_nontrivial s).mpr ⟨zero, by linarith, hone⟩
  exact (canonical_li_summable_right_half_plane summable s hhalf) xiZero
