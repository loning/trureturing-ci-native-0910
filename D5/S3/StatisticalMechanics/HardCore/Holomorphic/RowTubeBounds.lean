/- GID: D5/S3/StatisticalMechanics/HardCore/Holomorphic/RowTubeBounds
   generality: G
   mirror-B: D5/B/S3/StatisticalMechanics/HardCore/Holomorphic/RowTubeBounds
...
import D5.S3.StatisticalMechanics.HardCore.Holomorphic.TubeEstimates

set_option autoImplicit false
set_option relaxedAutoImplicit false

noncomputable section
namespace D5.S3.StatisticalMechanics.HardCore.Holomorphic.RowTubeBounds
...
theorem row_tube_bounds {ι : Type*} [DecidableEq ι]
  ...
  have hAd : ‖A-(A0:ℂ)‖ ≤ 20000*delta := by
    ...
    calc
      ...
      _ ≤ epsilon*16+3*(6400*delta) := add_le_add
        ...
        (mul_le_mul (by simpa [Complex.norm_real, Real.norm_eq_abs,
  ...
  · intro j hj
    ...
    have hQ0 : 0 ≤ Q0 ∧ Q0 ≤ 3 := by
      ...
      nlinarith [(hc j hj).2.1, mul_le_mul_of_nonneg_left (hr j hj).2 cj.2.1,
    ...
    have hQ : ‖Qj‖ ≤ 9 := by
      have h := norm_triangle_sub Qj (Q0:ℂ)
      have hqn : ‖(Q0:ℂ)‖ ≤ 3 := by
    ...
    have hNum : ‖(-A*Qj)-((-(A0*Q0):ℝ):ℂ)‖ ≤ 180900*delta := by
      ...
      calc
        ...
        _ ≤ (20000*delta)*9+3*(300*delta) := add_le_add
          (mul_le_mul hAd hQ (norm_nonneg _) (by positivity))
