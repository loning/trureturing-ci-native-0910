/- GID: D5/S3/StatisticalMechanics/HardCore/Holomorphic/NormalizedPartitionLog
   generality: G
   mirror-B: D5/B/S3/StatisticalMechanics/HardCore/Holomorphic/NormalizedPartitionLog
...
import D5.S3.StatisticalMechanics.HardCore.Holomorphic.PartitionLogCocycle

set_option autoImplicit false
set_option relaxedAutoImplicit false

noncomputable section
namespace D5.S3.StatisticalMechanics.HardCore.Holomorphic.NormalizedPartitionLog
...
open D5.S3.StatisticalMechanics.HardCore.Holomorphic.AdaptiveComplexNeighborhood
open D5.S3.StatisticalMechanics.HardCore.Holomorphic.FiniteGridZeroFree
open D5.S3.StatisticalMechanics.HardCore.Holomorphic.PartitionLogCocycle
...
theorem normalized_log_hasDerivAt (V : Finset Point) (z : ℂ) (hz : z ∈ ActivityTube) :
  ...
  simpa only [normalizedLog, complete_erases, he, sub_zero] using h
...
private theorem ordered_im_bound (V : Finset Point) (l : List Point)
  ...
  | cons v l ih =>
      ...
      calc
        _ ≤ |(Complex.log (increment V v z)).im| + |(orderedLog (V.erase v) l z).im| := abs_add _ _
...
theorem normalized_log_delete (V : Finset Point) (v : Point)
  ...
  by_cases hv : v ∈ V
  · have hnodup : (v :: (V.erase v).toList).Nodup := by simp
...
theorem normalized_log_volume_bounds (V : Finset Point) (z : ℂ) (hz : z ∈ ActivityTube) :
  ...
  have hz3 : ‖z‖ < 3 := by
    ...
    have ht := norm_triangle_sub z (lam : ℂ)
    have hl : ‖(lam : ℂ)‖ = lam := by
      simp [Complex.norm_real, Real.norm_eq_abs, abs_of_nonneg hlam.1]
    rw [hl] at ht
    have hw : D5.S3.StatisticalMechanics.HardCore.Holomorphic.TubeEstimates.epsilon < (1 / 4 : ℝ) := by
      norm_num [D5.S3.StatisticalMechanics.HardCore.Holomorphic.TubeEstimates.epsilon]
    linarith [hlam.2]
...
end D5.S3.StatisticalMechanics.HardCore.Holomorphic.NormalizedPartitionLog
