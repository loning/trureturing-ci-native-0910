/- GID: D5/S3/StatisticalMechanics/HardCore/Holomorphic/TubeEstimates
   generality: G
   mirror-B: D5/B/S3/StatisticalMechanics/HardCore/Holomorphic/TubeEstimates
...
import D5.S3.StatisticalMechanics.HardCore.Holomorphic.TypedJacobian

set_option autoImplicit false
set_option relaxedAutoImplicit false

noncomputable section
namespace D5.S3.StatisticalMechanics.HardCore.Holomorphic.TubeEstimates
open scoped BigOperators
open D5.S3.StatisticalMechanics.HardCore.Holomorphic.AffineChart
...
theorem inverse_tube (a b x : ℝ)
  ...
  have hnorm := normalized_inverse_bound (x:ℂ) k E (6*delta)
    ...
    (by simpa [k, Complex.norm_real, Real.norm_eq_abs, abs_of_nonneg hk.1] using hk.2)
    (by rw [show (1:ℂ)-k = ((1-a*x/b:ℝ):ℂ) by push_cast; rfl];
        simp only [Complex.norm_real, Real.norm_eq_abs, abs_of_nonneg (by linarith [hk.2])];
        linarith [hk.1])
    ⟨by positivity, width_arithmetic.2.2.2.1⟩ he
  ...
  | @insert j s hj ih =>
      ...
      calc
        _ ≤ ‖x j-r j‖*‖∏ k ∈ s, x k‖+
          ...
          simpa only [norm_mul] using norm_add_le _ _
...
theorem product_four_bound {ι : Type*} [DecidableEq ι]
  ...
  · refine ⟨Finset.prod_nonneg hrn, ?_⟩
    ...
    simpa using h
  · have h := product_difference s x (fun j => (r j:ℂ)) (100*delta) (by positivity)
      hx (fun j hj => by simpa [Complex.norm_real, Real.norm_eq_abs,
        abs_of_nonneg (hrn j hj)] using (hr j hj).2) he
    have hc : (s.card:ℝ)*(2:ℝ)^s.card ≤ 64 := by
      nlinarith [pow_nonneg (by norm_num : (0:ℝ)≤2) s.card]
    push_cast at h ⊢
    exact h.trans (by nlinarith [mul_le_mul_of_nonneg_right hc
      (show 0≤100*delta by positivity)])
...
end D5.S3.StatisticalMechanics.HardCore.Holomorphic.TubeEstimates
