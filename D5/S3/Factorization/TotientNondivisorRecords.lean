...
/-- The least `k ≥ 2` dividing `0^N + 1^N + ... + (k-1)^N`. -/
...
/-- `51` is not the least power-sum divisor at exponent `1275120`. -/
theorem A095366_1275120_ne_51 : A095366 1275120 ≠ 51 := by
  intro heq
  have hne : {k : ℕ | PowerSumDivisor 1275120 k}.Nonempty :=
    ⟨53, PowerSumDivisor_1275120_53⟩
  have hmem := Nat.sInf_mem hne
  change PowerSumDivisor 1275120 (A095366 1275120) at hmem
  rw [heq] at hmem
  exact powerSum_1275120_not_dvd_51 hmem.2
