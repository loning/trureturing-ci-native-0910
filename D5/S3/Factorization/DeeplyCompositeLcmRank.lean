...
/-- `DivPlusPrecedes n m` says that the least positive integer where divisibility
by `n` and `m` differs divides `n`. This is the record ordering encoded by
`Div+` in OEIS A095848 (Switkay, 2023-08-27). -/
def DivPlusPrecedes (n m : ℕ) : Prop :=
  ∃ d : ℕ, 1 ≤ d ∧ d ∣ n ∧ ¬ d ∣ m ∧
    ∀ e : ℕ, 1 ≤ e → e < d → (e ∣ n ↔ e ∣ m)

/-- A deeply composite number is a positive left-to-right minimum for the
divisor-vector ordering. -/
def DC (n : ℕ) : Prop :=
...
/-- For a positive first argument, a witness for `DivPlusPrecedes` can be
chosen no larger than that argument. -/
theorem divPrecedes_iff_first_difference {n m : ℕ} (hn : 1 ≤ n) :
    DivPlusPrecedes n m ↔
      ∃ d : ℕ, 1 ≤ d ∧ d ≤ n ∧ d ∣ n ∧ ¬ d ∣ m ∧
        ∀ e : ℕ, 1 ≤ e → e < d → (e ∣ n ↔ e ∣ m) := by
  constructor
  · rintro ⟨d, hdpos, hdn, hdm, hfirst⟩
    exact ⟨d, hdpos, Nat.le_of_dvd hn hdn, hdn, hdm, hfirst⟩
  · rintro ⟨d, hdpos, _hdnle, hdn, hdm, hfirst⟩
    exact ⟨d, hdpos, hdn, hdm, hfirst⟩

/-- The universal record-low definition is equivalent to saying that every
positive predecessor belongs to the finite set of vectors preceded by `n`. -/
theorem dc_iff_left_to_right_minimum (n : ℕ) :
    DC n ↔
      1 ≤ n ∧
        (Finset.Ico 1 n).filter (DivPlusPrecedes n) = Finset.Ico 1 n := by
  constructor
  · rintro ⟨hn, hrecord⟩
    refine ⟨hn, Finset.ext fun m => ?_⟩
    simp only [Finset.mem_filter, Finset.mem_Ico]
    constructor
    · exact fun hm => hm.1
    · exact fun hm => ⟨hm, hrecord m hm.1 hm.2⟩
  · rintro ⟨hn, hrecord⟩
    refine ⟨hn, fun m hmpos hmn => ?_⟩
    have hm : m ∈ (Finset.Ico 1 n).filter (DivPlusPrecedes n) := by
      rw [hrecord]
      exact Finset.mem_Ico.mpr ⟨hmpos, hmn⟩
...
private theorem recordsBelowL_succ (k : ℕ) :
  ...
  ext d
  simp only [recordsBelowL, recordBand, Finset.mem_filter, Finset.mem_Ico,
    Finset.mem_union]
  have hmono := L_mono_succ k
  constructor
  · rintro ⟨hd, hdc⟩
    by_cases hdk : d < L k
    · exact Or.inl ⟨⟨hd.1, hdk⟩, hdc⟩
    · exact Or.inr ⟨⟨le_of_not_gt hdk, hd.2⟩, hdc⟩
  · rintro (⟨hd, hdc⟩ | ⟨hd, hdc⟩)
    · exact ⟨⟨hd.1, hd.2.trans_le hmono⟩, hdc⟩
    · have hLpos : 1 ≤ L k := Nat.lcmUpto_pos k
      exact ⟨⟨hLpos.trans hd.1, hd.2⟩, hdc⟩
...
theorem rank_nth (r : ℕ) : rank (a r) = r + 1 := by
  calc
    rank (a r) = ((Finset.range (a r + 1)).filter DC).card := by
      unfold rank
      apply congrArg Finset.card
      ext n
      simp only [Finset.mem_filter, Finset.mem_Icc, Finset.mem_range]
      constructor
      · rintro ⟨⟨hnpos, hnle⟩, hnDC⟩
        exact ⟨by omega, hnDC⟩
      · rintro ⟨hnlt, hnDC⟩
        exact ⟨⟨hnDC.1, by omega⟩, hnDC⟩
...
#print axioms divPrecedes_iff_first_difference
#print axioms dc_iff_left_to_right_minimum
