...
private def InS (x : Nat) : Prop := x = 3 ∨ x % 3 = 1
...
/- We encode the integer equality `x = a'(i) * a'(j) - a'(k)` as the
subtraction-free equality `x + a'(k) = a'(i) * a'(j)`. -/
        ...
        0 < x ∧ a (n - 1) < x ∧
          ∀ i j k, 1 ≤ i → i ≤ j → j ≤ k → k ≤ n →
            x + (if k = n then x else a k) ≠
              (if i = n then x else a i) * (if j = n then x else a j)} (a n)

private theorem admissible_iff_literal {a : Nat → Nat} {n x : Nat} :
    Admissible a n x ↔
      0 < x ∧ a (n - 1) < x ∧
        ∀ i j k, 1 ≤ i → i ≤ j → j ≤ k → k ≤ n →
          x + (if k = n then x else a k) ≠
...
private theorem isLeast_admissible_iff {a : Nat → Nat} {n z : Nat} :
  ...
  constructor
  · intro h
    exact ⟨admissible_iff_literal.mp h.1,
      fun y hy ↦ h.2 (admissible_iff_literal.mpr hy)⟩
  · intro h
    exact ⟨admissible_iff_literal.mpr h.1,
      fun y hy ↦ h.2 (admissible_iff_literal.mp hy)⟩
...
private theorem target_memS {n : Nat} (hn : 1 ≤ n) : InS (target n) := by
  ...
  by_cases hn2 : n = 2
  · simp [target, InS, hn2]
  simp [target, InS, hn1, hn2]
  omega

private theorem memS_small {x : Nat} (hx : InS x) (h : x ≤ 3) : x = 1 ∨ x = 3 := by
...
/-- Ordered values in `{3} ∪ {x | x % 3 = 1}` cannot exclude another member
at least four. The exceptional residue-zero value `3` is controlled by order. -/
theorem residue_protection {x u v w : Nat}
    (hx4 : 4 ≤ x) (hx : x = 3 ∨ x % 3 = 1)
    (hu : u = 3 ∨ u % 3 = 1) (hv : v = 3 ∨ v % 3 = 1)
    (hw : w = 3 ∨ w % 3 = 1) (huv : u ≤ v) (hvw : v ≤ w) :
    x + w ≠ u * v := by
...
/-- Every integer at least seven outside `{3} ∪ {x | x % 3 = 1}` has an
ordered exclusion witness inside that set and strictly below the integer. -/
theorem witness_of_not_memS {y : Nat} (hy : 7 ≤ y)
    (hyS : ¬ (y = 3 ∨ y % 3 = 1)) :
    ∃ u v w, (u = 3 ∨ u % 3 = 1) ∧ (v = 3 ∨ v % 3 = 1) ∧
      (w = 3 ∨ w % 3 = 1) ∧ u ≤ v ∧ v ≤ w ∧ w < y ∧ y + w = u * v := by
  have hymod : y % 3 ≠ 1 := fun h ↦ hyS (Or.inr h)
...
private theorem target_unique (a : Nat → Nat) (ha : SatisfiesLiteralRule a) : a = target := by
  ...
  | h n ih =>
      ...
      have hhist : ∀ r, 1 ≤ r → r < n → a r = target r := by
        intro r _ hrn
        exact ih r hrn
      have hadm := isLeast_admissible_iff.mpr (ha.2.2 n hn2)
...
theorem sequence_recurrence (n : Nat) (hn : 2 ≤ n) :
    IsLeast {x |
      0 < x ∧ sequence (n - 1) < x ∧
        ∀ i j k, 1 ≤ i → i ≤ j → j ≤ k → k ≤ n →
          x + (if k = n then x else sequence k) ≠
            (if i = n then x else sequence i) *
              (if j = n then x else sequence j)} (sequence n) := by
