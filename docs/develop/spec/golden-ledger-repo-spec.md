...
规范修正:该行是 partial-open historical projection;执行顺序遵循 CLAUDE.md「先库后证与写作尽调」 与 A17,仅当第三方生态检索与本地证明均失败后才可记录 AxiomDebt。
...
**无 status 字段（诚实分栏）。** v1 **没有**任何机器能派生候选的生命周期（candidate / consumed / discarded）：消费发生在 theorize 运行时，仓内无收据可判。手写状态由 SL-006 明禁，而设一个无人能判的字段即 CLAUDE.md「义务与证据强度」所禁的冒领，故本类**不设**该字段。当前唯一状态即「文件在 `Problems/` 内」，其语义恰为「已入池的候选」，不表任何真值、不表已被消费、不表未被消费。机器派生的消费/退役状态记 `open(案号待开)`。
...
**投影：Markdown 标记。** `CanonicalMarkdownWriter` 在该 `Describe` 的正文后发射一句人读的 `*Resolves.* ` 与一条机读注释 `<!-- scribe-open-problem-resolution-v1 {"problem_slug":…,"declaration_gid":…,"resolution_kind":…} -->`，键集恰为上述三项，且那句人读散文与 marker 点名**同一个**声明，故读者与机器不会各读到一个答案。它是 `.md` 投影的一部分——`Meta/FILEMAP.toml` 记 `Blueprint/**/*.md` 为 `kind=generated`、`produced_by=ScribeEmitter`、`verified_by=ScribeEmitter`——故**不是**第二真源（CLAUDE.md「投影的判定与保护边界」四项合取：producer 受治理、输入闭包受治理、可由 `make emit` 逐字节重建、无独立权威）。**如实**：`emit-check` 目标已随投影守卫整族退役，故标记与 claim 的一致由「重跑 `make emit` 即得同字节」承担，不由一道常驻门承担；手改 `.md` 是 CLAUDE.md「单向产线与冻结成员」所禁的第二真源，正解是改 `.scribe.cs` 后重发。
...
**升提律**:TheoryErratum 专用 schema/lint 规则仅在第二个同构实例出现,或已有路由被证实失效时再立;届时再机器化触发分类、唯一案件复用、issue/送达/重试等尚由评审守护的规范性动作,依 CLAUDE.md「分类、生长与减法」不预建空壳。〔守护:**元准则+评审**·第二同构实例或路由失效是升提前提;当前不冒领专用 schema/lint〕
