...
**A22 preflight 与 CI 三门同器(v7.31,CLAUDE.md 器律②之 spec 形)** **§1 目的与等价域(S5,C10).** 本地 `make preflight BASE=<sha>` 是 PR 模式实质判词的早反馈,不强制作提交前闸;CI 三门仍是唯一准入权威(器律②)。等价域恰为同一 `(tree(M),BASE,H)` 上、PR 事件下 engineering、lean-inspect、admission 三段实质判词;不含 GitHub merge-ref 等待、checkout、剥远端、cache、artifact、toolchain 与 runner 可用性,也不含 advisory 信号 PR head branch grammar 及 dev push 事后检测 `capacity-audit`;push 不进入 §6 等价域。**§2 三段判词体与属主(S1,C6,C7,C8).** 按器律①分层,三段都以当前已检出候选树 `HEAD` 为输入且接口封闭:①`make -C tools engineering` 属工具层门,其段脚本是 solution locked restore 的 canonical owner,始终执行 `dotnet restore StrataLint.sln --locked-mode` 与两项反证工程的 locked restore,再执行 `tools/scripts/dotnet-build.sh` 的 `--warnaserror` build → `tools/scripts/workflow/engineering-test-execution-harness.sh` 直接调用与 `tools/Makefile:22-23` 同一 canonical `dotnet run … EngineeringScope` 入口对 `HEAD^1..HEAD` 作增量测试选择 → 直接调用 `tools/scripts/stratalint-selftest.sh` → CompileFailProof(`build rc≠0 ∧ 输出含 CS7036 ∧ 输出含 MetaClear`,取 `.github/workflows/ci.yml:171-181` 的 CI 现行强断言)→ BannedApi 反证(逐 marker 对齐 `RS0030` 且无其它 error);经 §7 内容地址核对的预建输出只可省编译,不省 solution/反证工程 locked restore 或 compile assets/package 恢复。②`make lean-inspect` 属内容层门,顺序为 `tools/scripts/report/lean-report-input.sh address` → 按 §7 复用或单次生产报告 → `tools/scripts/workflow/scribe-content-checks.sh REPORT [SCRIBE_DLL] HEAD^1`,输出报告及 sidecars 供 admission。③`make admission` 属内容层门,以 lean-inspect 的 `REPORT` 及可选已核址 judge DLL 为输入,调用 `.github/scripts/harness-gate.sh --candidate . --base $(git rev-parse HEAD^1) --candidate-lean-report REPORT [--judge-dll <DLL>]`,内部顺序 restore → §7 所定 build-or-cached-judge → `check --protected-base` → `filemap-conform` 不变。每段开头 fail-fast 校验全部输入,内部只解 `HEAD`/`HEAD^1`(SL-030 允许集),并允许只对字面 `HEAD` 读取已检出提交对象的父元数据;此许可不授权物化或执行第二父代码。段不接受外传 BASE/HEAD 覆盖,传入值只可核对,可安全重跑;所有出口按 §3 发射具名哨兵。段脚本住 `tools/scripts/workflow/**` 的 SL-030 判官面;`tools/scripts/preflight.sh` 留在判官面之外,因为它须 `git worktree add --detach … <M>` 物化非字面 HEAD 的合成候选。`EngineeringScope` 在 `tools/StrataLint.EngineeringScope/Program.cs:46-53` 执法 `--base == 已检出 HEAD^1`,是该等式唯一执法者;`.github/scripts/harness-gate.sh` 不重复执法。raw 必须在任何会折叠退出码的内层 make 之前取得:执行 harness 直接调用上述 canonical `EngineeringScope`/selftest 入口,或由执行属主先发 canonical 子步骤结果;外层不得猜日志,也不得把内层 make 的 `2` 一概解释为基础设施失败。三个目标名是选定接口而非目的唯一推出,不设 `MODE`/`CI`/`PR` 选择器。**§3 退出码与段证据三层契约(S1,S5,S6,C6,C10).** raw 层按段、按失败类统一归一:成功=`0`;候选检查失败或 preflight 合成 merge 冲突=`1`;输入/对象/路径/无法核址、restore/build/git/report 等基础设施失败=`2`;仅 admission 的 SL-022 保护面变更=`3`。任意子进程自身其它码不得外泄,须在内层 make 折叠前归入上述 `1/2`。`outcome` 闭集逐字为 `passed|candidate-check-failed|merge-conflict|protected-surface-change|missing-required-input|missing-object|non-commit-input|invalid-base-format|dirty-worktree|invalid-path|engineering-address-verification-failed|lean-report-address-verification-failed|scribe-address-verification-failed|judge-address-verification-failed|report-evidence-mismatch|subprocess-infrastructure-failed|merge-materialization-failed|parent-mismatch|base-equals-head`;缺显式必需输入映射 `missing-required-input`,缺对象映射 `missing-object`,对象非 commit 映射 `non-commit-input`,BASE 非 lowercase 40-hex 映射 `invalid-base-format`,脏树映射 `dirty-worktree`,非法路径映射 `invalid-path`,地址计算器不可用或 DLL attestation malformed 依段映射相应 `*-address-verification-failed`,报告交接证据不一致映射 `report-evidence-mismatch`,报告生产或回落构建失败映射 `subprocess-infrastructure-failed`,均 raw `2`;§7 可核验但地址不符的缓存 miss 不产生失败 outcome。每个段脚本安装 `EXIT` trap,在成功、候选红、基础设施红及开头早退的所有出口恰发一条 stdout 段哨兵;preflight 在三段前失败也恰发一条 `segment=preflight` 哨兵。段证据唯一 canonical schema 为 `pfci-segment-evidence-v1`,每段 stdout 恰发一行完整 canonical JSON,不得使用引用形;键恰按以下次序且不得省略:`schema_version`,`segment`,`event`,`merge_commit`,`tree`,`base`,`source_head`,`raw_rc`,`outcome`,`report_input_address`,`report_sha256`,`judge_source_address`,`scribe_source_address`,`selected_test_ids`,`ordered_check_ids`;编码固定为无 BOM UTF-8 单行、键间无可选空白,OID 为 lowercase 40-hex、SHA-256 为 lowercase 64-hex,`raw_rc` 为 JSON integer,地址为 canonical producer 发出的非空字符串,数组元素均为字符串。`schema_version` 逐字为上述版本,`segment` 只取 `engineering|lean-inspect|admission|preflight`;`event` 的**来源**为:CI 壳按事件类型映射(`pull_request_target`→`PR`、`push`→`push`)并作为必需输入传入段;preflight 固定传 `PR`,其父数校验只判 M 的两父,**不改变任何检查集**〔勘正 RB3:原文规定了 `event` 的取值域却未规定其**来源**,使段无从判断该信任谁;**已由实施闭合**〕。`event` 只取 `PR|push`,**唯一例外**是段在事件校验完成前早退——含 `EVENT` 缺失或非法、以及证据库/编码器在事件赋值前即失败——此时 `event` 取 JSON `null`,且该情形 `raw_rc` 恒为 `2`、`outcome` 属基础设施或缺输入类,**不得**出现在 `raw_rc` 为 `0`/`1`/`3` 的出口。〔勘正 C5,2026-09-07:原文「只取 `PR|push`」与同段「开头早退的所有出口恰发一条哨兵」及「键不得省略」三者合取在事件尚未确定时**不可同时满足**,是本条的内在矛盾而非实现选择;实测早退出口矩阵 121 条中 9 条落此例外(EVENT 自身无效 6、库/编码器在赋值前失败 3)。本例外**不放宽任何判词**:`null` 只在失败出口出现,成功路径恒为 `PR|push`。〕身份派生只对字面 `HEAD` 调用一次 `git rev-list --parents -n 1 HEAD`,其输出形为 `<commit> <p1> [<p2>]`;`merge_commit` 取 commit、`tree` 取 `HEAD^{tree}`,`base` 取第一父、`source_head` 取第二父。PR 事件父数必须恰为 `2`,否则该段 raw `2`、`outcome=parent-mismatch`;push 事件父数为 `2` 时 `source_head` 取第二父,否则为 canonical `null` 且不算证据缺失,push 仍完整执行三门。字段 required/null 由 `segment × 失败阶段` 决定:所有出口都 required 真实的 `schema_version`/`segment`/`event`/`raw_rc`/`outcome`;已经取得或核验的身份与证据填真值,尚未产生、未执行或未能核验者填 canonical `null`,不得用 `[]` 冒充尚未发生的测试选择。engineering 在测试选择实际发生后才 required `selected_test_ids`,并由 `EngineeringScope`/执行 harness 产出、按 UTF-8 字节序升序去重;确实选择而集合为空才可用 `[]`;报告及 judge/scribe 地址不适用而为 `null`。engineering 的 `ordered_check_ids` 仅在对应子步骤实际发生后填实际有序值。lean-inspect 的 `report_input_address`/`report_sha256` 分别在取得输入地址、核验或生产报告后填真值,`scribe_source_address` 仅在 scribe 来源核验完成后填真值,`ordered_check_ids` 仅在检查实际发生后填实际有序值;judge 地址与测试选择为 `null`。admission 遇 `REPORT` 缺失时报告及 scribe 字段为 `null`;取得报告后须先将 `report_input_address`/`report_sha256` 与 lean-inspect 段证据逐字核对,一致才继承其中的 `scribe_source_address`,再于 judge 核址及检查实际发生后分别填 judge 地址与有序 check id;`stratalint-lean-report-provenance-v1` 保持 `tools/scripts/lean-report-pair.sh:189-193` 所示 exact-key 十键不变,将来若把 scribe 地址放入 provenance 必须升版,并在 L2 同列 producer/validator/缓存地址变化及整报告重产代价。preflight 只 required 五个通用失败字段,其它值随取得阶段填真值或 `null`。`ordered_check_ids` 按规范执行序号升序,同序号按 UTF-8 字节序,不按完成时间。上述是合法失败记录的条件 schema;只有该段等价比较所需身份与证据均非 `null` 才是完整记录,合法早退记录若证据不足在 §6 仍判 `INCOMPLETE`。make 层只承诺 `0=成功`、`2=配方失败`,读数于 `0de1194cec` 的 CLAUDE.md 导航实测配方 `exit 4` 与 `exit 69` 均得 make `2`;原因只从哨兵读取,不得声称 make 透传 raw `1/3`;末层才映射 GitHub check conclusion。admission raw `3` 时段成功(make `0`),哨兵记 `raw_rc=3`,summary 逐字为 `protected-surface change (SL-022); content checks passed`;raw `1/2` 时 make `2`。**§4 CI 壳(S1,C1,C3).** 三个 required job 名逐字保持 `Candidate harness engineering checks`、`Canonical Lean report production`、`Content-addressed dev baseline admission`;`strict=false`、`enforce_admins=false` 与分支保护不动,runner/needs/artifact/cache 拓扑不动,`lean-inspect` 留在 `ubuntu-24.04-arm` 的理由标 `ASSUMED-UNVERIFIED` 而不借统一之名换 runner。每个 job 的判词阶段恰一行调用其对应 make 目标;YAML 继续承载 merge-ref 等待及不可用时 fail-closed、剥远端、工具链、cache restore/save、artifact 上传下载与 summary。域外保留 engineering job 的 push-only `make -C tools capacity-audit` 一行,因为它是既有 dev push 事后全仓检测,不进 PR 等价域、preflight 不跑、不设 mode 且不新建 workflow;域外亦保留 branch grammar advisory 步骤,因为其输入是运输侧 event payload 的 head ref,detector fault 令 job 红属于基础设施红而非实质判词,是否删除记 open。不新增任何读取 `.github/workflows/**` 的测试(器律⑦′);YAML 形状只由评审与真跑守。**§5 preflight 对象域与造树算法(S2,S6,C2,C7).** 输入 `BASE` 缺失时 raw=`2`,`outcome=missing-required-input`;其值必须是显式 lowercase 40-hex,否则 raw=`2`,`outcome=invalid-base-format`;本地对象库缺该对象时 raw=`2`,`outcome=missing-object`,对象非 commit 时 raw=`2`,`outcome=non-commit-input`;不接受 refname、不 fetch、不读 `origin/dev`。候选 H 是源工作树 `HEAD`;`BASE == H` 时 raw=`2`,`outcome=base-equals-head`,但 BASE 不要求为 H 的祖先,须删除 `tools/scripts/preflight.sh:60` 的 `merge-base --is-ancestor`,因为它使冲突分支不可达并等价于 `strict` 局部复辟。源树 tracked 或 untracked delta 任一非空时 raw=`2`,`outcome=dirty-worktree`,因为被判对象必须是提交,而 `scribe-content-checks` 现读 `git ls-files --others`,脏树会使两种对象口径并存。造树严格为 `git merge-tree --write-tree BASE H`;语义 merge 冲突时 raw=`1`,`outcome=merge-conflict`,与 CI merge ref 不可用 fail-closed 同义;语义上不可合并(如 unrelated histories)与 merge 冲突同类,raw=`1`、`outcome=merge-conflict`;**仅**工具侧故障(git 子进程异常、tree 写入失败、对象库不可读)raw=`2`、`outcome=merge-materialization-failed`〔勘正 RB4:原文把「tree 不可得」一律归 raw `2`,混淆了「候选与 base 语义上合不了」(候选问题,raw 1)与「工具坏了」(基础设施,raw 2),违三层退出契约的分类原意〕;再以 `git commit-tree <tree> -p BASE -p H` 得 M,author/message 等元数据不入判词,M 无 ref且可被 GC。物化之前只读取 M 提交对象的父列表(如 `git rev-list --parents -n 1 M`,M 为 preflight 自造对象),要求 commit 后恰有两个 parent OID 且依次逐字等于 BASE、H;基数、顺序或任一字节不符时 raw=`2`,`outcome=parent-mismatch`,不得进入三段。核对通过才 `git worktree add --detach <tmp> M`;绝不物化 BASE,符合第 19 条 base 判官禁令与 SL-030 动词表,一切出口清理 tmp。2026-09-05 对 8 个 open PR(`#5576 #5575 #5572 #5569 #5568 #5567 #5564 #5562`)的读数是 `git merge-tree --write-tree <merge^1> <merge^2>` 所得 tree OID 与 GitHub `refs/pull/N/merge^{tree}` 逐个相等,采集于 `0de1194cec` 对象库与本机 git `2.50.1`;CI runner 的 git 版本及 merge 策略标 `ASSUMED-UNVERIFIED`。**三段必须钉定同一个 merge 对象**〔勘正 RB6:CI 三个 job 各自解析可变的 `refs/pull/N/merge`,GitHub 会在 base 前进时重算该 ref,故三段可能判到不同的树。要求:每个 run 只解析一次并把不可变 merge OID 传给三段,或每段对 expected `(tree, base, source_head)` 三元组 fail-closed;L2 的收据须断言**三段哨兵的该三元组逐字相同**。**已实测满足**:run `34050041481` 三段 `base` 均 `1fd59d6703`、`source_head` 均 `a8c874f6dd`、`tree` 均 `8982d5e675`〕。在 tmp 内严格顺序执行 `make -C tools engineering` → `make lean-inspect` → `make admission`,lean-inspect 报告原样交 admission 而不二次生产,三段哨兵的 `base` 字段须逐字等于 BASE。preflight 是 CI 运输管道的严格子集与三段实质判词的精确并集;总退出为三段皆 make `0` 则 `0`,任一非零则返回该段 make 码。**§6 本地=CI 的机器可判等价(S5,C10).** 比较只读 §3 的完整 canonical JSON:键 `K=(tree,base,source_head,segment,event,judge_source_address,scribe_source_address,report_input_address)`,其中本地与 CI 均须为 `event=PR`;比较证据元组为 `(raw_rc,outcome,report_sha256,selected_test_ids,ordered_check_ids)`;`merge_commit` 留作已检出对象的审计身份而不入 K。判定优先序写死:①任一侧缺哨兵、schema 键、该失败阶段已应取得的身份,或该段等价比较所需证据为 `null`,先判 `INCOMPLETE`;合法失败记录不因 schema 合法而自动完整;②两侧 K 均完整但任一字段不同,判 `NOT_COMPARABLE`;③同 K 且证据齐全后元组逐字全相等才判 `EQUAL`,任一不同判 `DIFFERENT`。push 不进入比较;时间、路径、runner OS/arch、cache hit/miss 不入键;同一 make 目标字节及两侧退出码相等都只是必要条件。反例集合分两维:①绕过包括 YAML 或 preflight 直调原语、残留 `make gate`/`make test` 入口、段接受外传 BASE、未核址 DLL/报告、`CI=true` 或 event 条件分叉改变测试集、脏树、preflight 判 lane HEAD;②检查被跳过包括 step `if:false`/`continue-on-error`、merge-ref 回退、make recipe 屏蔽非零、段内某 check 被删而总 rc 仍绿、哨兵消费者不运行、`pull_request_target` 下 PR 自身仍跑 base workflow。**§7 依赖闭包、缓存唯一处置与代价(S7,C8,C9).** harness 作用面仍只守真源与生成程序,不把 CI 运输投影或报告副本立成第二真源。`tools/scripts/report/lean-report-input.sh:381-386` 只覆盖 producer 闭包 entrypoints,`tools/scripts/report/lean-report-input.sh:396-399` 的 `reference_pattern` 只认 `.sh`,`tools/scripts/report/lean-report-input.sh:447-458` 的遍历也只沿该 pattern;entrypoints 虽含 `.github/workflows/ci.yml` 切片,却不解析 Makefile 目标,故须把 lean-inspect 段脚本加入 entrypoints或改为解析目标配方,否则新脚本不进入报告地址。缓存处置只有此表:①可完成核验但源码内容地址与候选源码闭包不同的预建 judge/scribe DLL 是 cache miss,不得执行,回落候选源码构建,不是错误;②地址计算器不可用、DLL attestation malformed、非法路径、明确要求存在的输入缺失,或回落构建自身失败,均 raw `2`,其中各段 `*-address-verification-failed` 只用于无法核验,不得用于已核验的地址不符;③lean-inspect 的缓存报告地址/SHA-256/sidecar/provenance 任一失配,按 A20 作为 miss 回落单次生产;admission 对交接 `REPORT` 缺失取 `outcome=missing-required-input`,与 lean-inspect 段证据核验不一致取 `outcome=report-evidence-mismatch`,二者均 raw `2` 且不得重产。cutover 改变报告地址,至少发生一次整报告重产;2026-08-21 历史读数约 84 分钟,只作观测不作预算,分层若多次换地址须逐层记账;若将 scribe 地址纳入 provenance,还须按 §3 记 producer/validator/缓存地址变化。preflight 另付合成 merge 的 worktree 物化成本。检测继续按第 20‴ 条增量:engineering delta-scoped、admission `HEAD^1..HEAD`、报告内容寻址;不建 base 报告、不跑 base 判官、不以全量 `make -C tools test` 替代 scoped。现 `.github/scripts/harness-gate.sh` 只查预建 DLL 存在,不足。冷树整报告时长标 `ASSUMED-UNVERIFIED`。**§8 退役闭包与消费者迁移(S3,C5).** 测试与消费者必须在其依赖接口首次改变之层同 PR 迁移;以下按测试方法归层,不得以整文件归层覆盖本规则。最终退役且不留 alias、stub、双读或过渡期的闭包为:`tools/scripts/local-harness-gate.sh`、`tools/scripts/workflow/math-gate.sh`、`tools/scripts/lib/admission-base-lib.sh`、根 `make gate`/`make test` 的现行入口与语义、`GATE_ARGS`、`--skip-engineering`;`make test` 的数学门用途改由 `make preflight BASE=<sha>` 承接,代价是多跑 engineering,PR run `33968664435` 的 engineering 读数为 `9m51s`。消费者读数于 `0de1194cec`:L2 随新增目标同步更新 `Makefile:7-13` 的目标清单、`Makefile:83-90` 的 help、`tools/tests/StrataLint.Tests/Commands/MakeWorkflow/MakeWorkflowTests.Dispatch.cs:537-589` 的 `GateCommandSignatures` 与 `:626` 的 `HelpRunsAndNamesEveryTarget`;`:590` 起的 `ScribeContentChecksHaveOneCanonicalCommandList` 随其依赖接口在 L3 迁移。L3 同步替换 `tools/tests/StrataLint.Tests/Commands/MakeWorkflowTests.cs:141-176` 两条、`MakeWorkflowTests.PreflightBase.cs`、上述 Scribe 钉子及 `tools/tests/StrataLint.ArchitectureTests/Determinism/BannedApiCoverageTests.cs:46-54`,删失真字面钉子而保留程序行为测试;`MakeWorkflowTests.HarnessGateChain.cs` 中直接钉旧 preflight 的 `:195 PreflightPreservesHarnessGateExitCodeAcrossTheFullChain`、`:251 PreflightUsesExplicitAncestorVerbatimWithoutMutatingGit`、`:322 PreflightRejectsLegacyMergeBaseResolutionContract`(含 `:331` 的 `merge-base --is-ancestor` 断言)、`:360 BaseResolutionFailureDiagnosticsMatchEachEntrypointContract` 整方法(含其 PreflightScriptPath 分支),以及上述四方法的 fixture/shim,同 PR 替换为新对象域、真实 make 折叠、哨兵与清理行为测试。L4 只退役届时仍依赖 local-harness-gate/math-gate 的方法,包括〔勘正 RB2:原文把该文件 `:14/:89` 的 scribe-content-checks 与 `:126` 的 harness-gate 列入 L4 退役,**误判** —— 这三个方法测的是 canonical 的 `scribe-content-checks.sh` 与 `harness-gate.sh`,二者在 L2 后**仍为现役判词入口**(lean-inspect 段调前者、admission 段调后者),**须永久保留,不得退役**;此处已从 L4 清单删除〕`:340/:383` 的 local-harness-gate 方法,不得把上述 L3 方法拖到 L4;并迁移 CLAUDE.md 器律①、**②**、⑥″、导航及第 19 条 `:319` 的全量 `make -C tools test` 现役说法〔勘正 RB5:原消费者表**漏列器律②**(「本地=CI 同一器」直接描述 `make preflight` 的语义,是 L3 重写的首要消费者);且 CLAUDE.md 的迁移**须按首变层拆**——**器律② 中描述 preflight 行为的语句随 L3**(preflight 重写时其语义即变),**器律①/⑥″ 中的 `make gate`/`make test` 语句随 L4**(旧入口退役时才失效);原文把 CLAUDE.md 整体归 L4,会使 L3 落地后规范文字与实现不符〕、`skills/codex-formalize/SKILL.md:35-41` 的 PATH 旧提取和 `skills/codex-theory-ingest/SKILL.md:148` 等旧引用。GateAuthority 数据定位须拆写:`Golden/gate-authority-roots.toml:32-53` 只覆盖 `local-harness-gate.sh/*` 六 root,`:4` 才是 `Makefile/gate`,`:8` 是 `Makefile/preflight`;L2 增三段 root,L4 删旧 root。GateAuthority 整机退役记 open:相关实现读数于 `0de1194cec` 共 642 行且 tracked tree 无门上消费者,L4 以 `git grep -n GateAuthority` 与逐文件 `wc -l` 复核。调用链在 L3 原子迁移:`skills/codex-formalize/SKILL.md:164-172` 的 deposit/cover 当前在 `:166,172` 调 preflight、到 `:176-182` 才 commit,须先捕获 PR BASE、形成唯一候选 commit并确认 clean后再 preflight;`tools/scripts/agent/land.sh:58` 当前 preflight 先于 `:66-70` commit,同样改序;`tools/scripts/workflow/playbook-workflows.sh:295-303` 须移除写树后内部 preflight并由提交后的调用者执行,或拆成不在同一调用中先写树再 preflight的接口;`skills/codex-theory-ingest/SKILL.md:322` 保持 commit 后并改用捕获的 PR BASE。四条调用链各有行为验收;PATH 共享若仍需,迁到一个可 source 的 canonical owner,禁止两个 skill 各抄一份。L4 另迁移 `tools/StrataLint.Scribe/Vendor/Katex/README.md:20-21` 与 `tools/StrataLint.Cli/Commands/Ledger/DagLedgerCommandPreparation.cs:46-51`;历史报告与自审负例字面不迁移。**§9 层序与验证义务(S4,C3,C4,C5,C11).** 每层独立过三门、不依赖后层,judge 与 content 分 PR(SL-029):L1(content,本次)只含 A16/A20 修订、A22 与 CHANGELOG,不实施、不 merge。〔勘正 2026-09-07:原文并列写有「不 commit、不 push、不开 PR」。那三项记录的是起草当时 sshx pass 的**授权边界**,不是本规范的规则——把一次会话的授权状态写进 spec 是范畴错误(CLAUDE.md 第 6 条:现状唯一,历史归 git)。且它与第 16 条「在 worktree 的工作及时提交并同步远程,不留长期未提交改动」直接冲突,实测该冲突已使本草案在工作树中悬空 2 天。故删去该三项,保留「不实施、不 merge」——前者是分面约束(SL-029:content 与 judge 不同 PR),后者是层序约束(每层独立过三门);**commit/push/开 PR 的时机由当次授权决定,不由本规范规定**。〕L2(judge)含三段目标与段脚本、`.github/workflows/ci.yml` 切换(workflow 与调用接口同 PR,服从器律⑦字面)、`lean-report-input` 闭包、gate-authority 增三 root、CompileFailProof 强断言及 §8 的 L2 消费者;程序行为测试覆盖三个目标、raw 归一、每出口完整 JSON、§7 缓存表、provenance/lean-inspect 段证据的 scribe 地址继承〔勘正 RB1(**已由实施闭合**):admission 与 lean-inspect 分属两个 job,原文未指定段证据的跨 job 载体。定为:lean-inspect 的 canonical 哨兵随 `raw-lean-reports` artifact 以**确定性路径**交接,admission 将其列为**必需输入**并强校验,缺失/不匹配 → `report-evidence-mismatch`、raw `2`。**已实测满足**:run `34050041481` 的 lean-inspect 与 admission 两段 `report_input_address` 与 `report_sha256` 逐字相同〕,并钉两例:内层候选失败与内层基础设施失败的外层 make 均为 `2`,但段哨兵分别保留 raw `1` 与 raw `2`;另验收预建输出地址有效但本机依赖缓存为空时仍完成 solution/compile assets/package locked restore。L2 器律⑦采用可核对的两张新文本收据:先让含同一份 workflow+脚本的候选 commit 成为 `integration-ci-pfci-tests` 的 base I,从独立 payload 分支向 I 开 PR,核对 workflow commit 逐字等于 I、三个 job 各执行一行对应 make且哨兵可见,作为第一收据;第二收据必须绑定实际 dev 同步增量。〔勘正 C6,2026-09-07:原文要求「以 I 为 base、以含 dev 的候选 J 为 head,令三门实际判 `I..J`」,即令候选判官去判 dev 的整段新增量。**该构造与分面门 SL-029 冲突**:实测 dev 最近 60 个提交的分面分布为 judge-only `7` / content-only `31` / MIXED `22`,**最长连续 judge-only 段仅 1 个提交**,故任何跨 ≥2 提交的 dev 增量几乎必然 judge/content 混面,delta 必触 `ADMISSION-PLANE-MIXED`(实测 run `34049091376`:两 job 同因此红,而 admission `RULE_REJECTED count=1` —— **除分面门外该 200 提交零违规**)。SL-029 自 2026-09-04 起现役,**晚于**器律⑦″(2026-08-30),故这是后立规则对前立规则的覆盖,非实现缺陷。该读数取自 60 提交窗口,**不构成「所有构造皆不可能」的定理**。**本草稿采纳 (c)**,理由逐条:(a) 接受异父等价收据与 (c) **实质相同**,但 (a) 的表述仍暗示「等价替代」而 (c) 直接改判语义,后者更准确;(b) 为本收据豁免 SL-029 会**降低检测**,触第 20 条红线「检测机制绝不降级」,**排除**;(c) 保留器律⑦″ 的**实质** —— 「判官逻辑与 dev 新增量的交互」由**在含该增量的并集树上跑完整判词**来验证,而 delta 方向本是达成该实质的一种手段、非目的。**故第二收据的判据修订为**:候选须在**含当时 dev 新增量的并集树**上取得三门全绿,并记录该 run 的 head、两个父提交与 tree;**不约束 delta 方向**。实测 run `34050041481`(`^1`=dev `1fd59d6703`、`^2`=候选 `a8c874f6dd`、tree `8982d5e675`)三门全绿,**满足本修订后的判据**。另记:`I..J` 方向的信息并未丢失 —— run `34049091376` 的 admission `RULE_REJECTED count=1` 表明**除分面门外该 200 提交在新判官下零违规**。〔**本条是 caller 对自己草稿的修正,owner 可推翻**:A22 v7.31 全文从未合入 dev(实测 `git show origin/dev:…spec.md | grep -c 'A22 …'` = 0),原 §9 写于 SL-029 现役之前,其字面构造经两条通道实测均不可满足;修正一个自相矛盾的草稿不构成新的治理授权,但 owner 保留改判权。〕〕记录该 run 的 base、候选 tree 与增量身份后才推进 integration 到 J。更新后的 payload PR 可另验新 base workflow 取源与调用形,但不得替代 `I..J` 第二收据;不得先推进 integration 再以 J 为 base 测 payload并冒充 dev 新增量收据。候选 PR 自身在 `pull_request_target` 下跑 base workflow 的绿灯不得算新文本已验;L2 候选改动 PR、首次 payload、workflow commit核对、三段执行、`I..J` 收据齐后才可向 dev 开 PR,合入后补首个 dev push 与下一 PR 两条真实 run 链接,红则按器律⑦⁗撤因;文件数超过读数于 `0de1194cec` 的 p75=`13` 须写不可再分理由。L3(judge)重写 preflight 合成 merge 编排器,同 PR 完成 §8 的 L3 测试与四条调用链迁移;行为测试覆盖 lowercase 40-hex、缺 BASE=`missing-required-input`、坏格式=`invalid-base-format`、缺对象=`missing-object`、非 commit、脏树=`dirty-worktree`、`BASE==H`、BASE 可 fast-forward 到 H、分叉无冲突、冲突 raw `1`、M 恰两父且父序 BASE/H、父列表核验失败=`parent-mismatch`、三段同 base、无 base checkout、所有出口清理及消费者先 commit/clean 后 preflight;另覆盖缺 REPORT、缺对象、报告生产失败、测试选择未发生四类早退,逐项断言已取得字段填真值、未取得字段为 `null` 且测试选择未发生时不是 `[]`。L4(judge)只完成 §8 届时仍存的旧入口/旧 root/按方法列明的测试、CLAUDE.md/skills 与两处现役说明迁移。L3、L4 各自以同样的实现收据与实际 dev 同步增量收据过三门并留两条 run 链接,不得借 L2 或彼此链接。**§10 不变量清单(S6).** C1 三 required check 名、`strict=false`、`enforce_admins=false`、分支保护不动;C2 任何路径不 checkout/restore/build/execute base,base 只以 `HEAD^1` SHA 与字节快照参与;C6 raw `0/1/2/3`、SL-022 rc=`3` 路径、`check`→`filemap-conform` 顺序、merge-ref 不可用 fail-closed 不变;C7 真实仓 git 引用只许 `HEAD`/`HEAD^1`,preflight BASE 为显式 lowercase 40-hex;允许读取已检出提交对象的父元数据,不授权物化或执行第二父代码。**§11 诚实分栏与 open(S7,C10).** 机器可判硬项为段输入校验与 raw 归一、每出口恰一行完整 canonical JSON、条件 required/null、哨兵 `base==BASE`、父列表与 M 父序、冲突 fail-closed、段顺序、报告/DLL 地址核对及程序行为测试;评审+真跑软项为 YAML 判词恰一行、run 链接、两维反例集合与层序。open 项逐项记 owner/验法/触发:`ASSUMED-UNVERIFIED` ARM runner 理由(owner=CI owner;验法=查历史裁决并作双架构真跑;触发=L2 切换前);`ASSUMED-UNVERIFIED` 变更集不同实际导致判词不同的次数(owner=L3 implementer;验法=在固定 PR/H/BASE/采集窗口内分别记录旧 lane `HEAD^1..HEAD` 与新 `M^1..M` 的 tree/base/delta/测试身份/判词再统计;触发=L3 集成真跑);`ASSUMED-UNVERIFIED` 冷树整报告时长(owner=report producer;验法=钉 tree/硬件/cache 的冷跑收据;触发=首次报告地址变化);`ASSUMED-UNVERIFIED` CI runner git 版本的 `merge-tree` 行为(owner=CI owner;验法=打印版本并逐 PR 对比 tree OID;触发=L3 集成真跑);`ASSUMED-UNVERIFIED` GateAuthority 整机退役(owner=L4 implementer;验法=调用闭包与 642 行复核;触发=L4 开工);`ASSUMED-UNVERIFIED` branch grammar advisory 删除(owner=owner;验法=运输价值与 detector-fault 真跑复核;触发=L2 评审);`ASSUMED-UNVERIFIED` SL-030 引擎规则 PR #5492 在 `0de1194cec` 时 OPEN(owner=#5492;验法=规则落地与动词表变异;触发=L2 触碰判官面前);`ASSUMED-UNVERIFIED` 两个 skill 之外动态生成的 `make gate`/`make test` 调用(owner=L4 implementer;验法=tracked/runtime 调用闭包普查;触发=旧入口退役前)。〔守护:**硬+软**·硬项由上述输入校验、哨兵与程序行为测试执法;软项由独立评审、integration 真跑及合入后 run 链接守;任一未验证项保持 `ASSUMED-UNVERIFIED`,不得折算为 `EQUAL`。〕
...
**封闭形状恰两项**：`problem_slug`（`ProblemSlugRef`，须合 `ProblemPoolPaths.IsCanonicalSlug`）与 `resolution_kind`（封闭字母表恰为 `proved|refuted`，`ResolutionKind` 枚举，构造期越界即 `ArgumentOutOfRangeException`）。**不含 declaration GID**：宿主 `Describe` 节点自己就持有那个声明（`DescribeStatement.LeanDeclaration`），把它再抄进 claim 即第二真源（第 6 条）。**owner 2026-09-06 裁决：不补 GID，页面以「Blueprint 记录 + 源行号」链接即终态，不是过渡形。**
...
- **v7.31**(2026-09-07):A22 规定 preflight 与 CI 三门同器的三段判词体、合成 merge 对象域、完整段证据、唯一缓存处置、机器可判等价、退役闭包及分层验证义务;A16 同步增量 engineering 与 `HEAD^1` admission 口径;A20 改以 `HEAD^1` 并将旧 `--skip-engineering`/gate 重跑/timing 交接语义交由 A22 收口;裁决:待 owner(本条由 sshx 六席面板 + 三席评审产出,产地见 PR 说明)。
