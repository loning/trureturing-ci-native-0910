# trureturing / D5 —— 仓库规范全卷 v7.23(定本:宪法·地层·编码·执法·八官·管线·治理·引导)
...
- **v7.23**(2026-09-05):A22 规定 preflight 与 CI 三门同器的三段判词体、合成 merge 对象域、完整段证据、唯一缓存处置、机器可判等价、退役闭包及分层验证义务;A16 同步增量 engineering 与 `HEAD^1` admission 口径;A20 改以 `HEAD^1` 并将旧 `--skip-engineering`/gate 重跑/timing 交接语义交由 A22 收口;裁决:待 owner(本条由 sshx 六席面板 + 三席评审产出,产地见 PR 说明)。
