...
theorem b4_source_full (n : ℕ) (t : Fin (n+1) → ℂ)
    (h0 : sourceThetaCoefficient 0 = 1) (ht : Function.Injective t)
    (hcrit : ∀ i, (qSource (n+2)).derivative.eval (t i) = 0) :
    (∑ i, -((n+2 : ℕ) : ℂ) * (qSource (n+2)).eval (t i) /
      (qSource (n+2)).derivative.derivative.eval (t i)) =
    ...
    (∑ i, -((n+2 : ℕ) : ℂ) * (qSource (n+2)).eval (t i) /
      (qSource (n+2)).derivative.derivative.eval (t i)) =
      -((n+1 : ℕ) : ℂ) / (12*((n+2 : ℕ) : ℂ)^2) *
        ((sourceThetaMoment 2 : ℂ)-3*(sourceThetaMoment 1 : ℂ)^2) :=
  ⟨b4_source n t h0 ht hcrit,
   (b4_source n t h0 ht hcrit).trans (b4_source_cumulant n)⟩

#print axioms b4_source_full
