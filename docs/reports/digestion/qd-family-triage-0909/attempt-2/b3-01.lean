...
def arrow (n : ℕ) (a : ℝ) (t eta : Fin (n+1) → ℝ) :
  ...
  fromBlocks (fun _ _ => a / ((n+2 : ℕ) : ℝ))
    (fun _ i => Real.sqrt (eta i)) (fun i _ => Real.sqrt (eta i)) (diagonal t)

theorem arrow_hermitian (n : ℕ) (a : ℝ) (t eta : Fin (n+1) → ℝ) :
    (arrow n a t eta).IsHermitian := by
  ext (i | i) (j | j) <;> simp [arrow, conjTranspose_apply, diagonal, eq_comm]
...
theorem arrow_schur (n : ℕ) (a : ℝ) (t eta : Fin (n+1) → ℝ)
  ...
  have hm : scalar (Fin 1 ⊕ Fin (n+1)) x - arrow n a t eta =
      fromBlocks (fun _ _ : Fin 1 => x-a/((n+2 : ℕ) : ℝ))
        (fun (_ : Fin 1) i => -Real.sqrt (eta i))
        (fun i (_ : Fin 1) => -Real.sqrt (eta i)) (diagonal v) := by
    ext (i | i) (j | j) <;> simp [arrow, scalar, diagonal, v, Subsingleton.elim i j]
...
theorem b3_charpoly_target (n : ℕ) (q : ℝ[X]) (a : ℝ)
  ...
  apply Set.Infinite.mono (Set.infinite_univ.diff (Set.finite_range t))
  intro x hx
  have hxt : ∀ i, x ≠ t i := by
    intro i hi
