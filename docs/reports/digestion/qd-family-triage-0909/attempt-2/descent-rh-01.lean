...
theorem source_real_descent (d : ℕ) (hd : 2 ≤ d) (h0 : sourceThetaCoefficient 0 = 1)
  ...
  have htop : (Q (sourceJensenPolynomial d) d).coeff d = 1 := by
    simp only [Q, coeff_reflect, revAt_le le_rfl, Nat.sub_self, comp_C_mul_X_coeff]
...
theorem source_real_antitone (h0 : sourceThetaCoefficient 0 = 1) :
  ...
  intro n
  change RealRoots (sourceJensenPolynomial (n+1+1)) → RealRoots (sourceJensenPolynomial (n+1))
...
theorem source_minimal_failure (h0 : sourceThetaCoefficient 0 = 1)
  ...
  · intro d hdd hdgood
    ...
    have hdpos : 1 ≤ d₀ := hd₀.1
    simpa only [Nat.sub_add_cancel hdpos] using ht hh
...
theorem reflect_nonnegative_of_negative (p : ℂ[X]) (d : ℕ)
  ...
  have hm := mul_neg_of_neg_of_pos hneg hn
  rw [div_mul_cancel₀ _ hn.ne'] at hm
  linarith only [hm]
...
theorem source_negative_descent (d : ℕ) (hd : 2 ≤ d)
  ...
  have hare : 0 < (alpha d).re := by
    have heq : alpha d = (((((d-1 : ℕ) : ℝ)/(d : ℝ)) : ℝ) : ℂ) := by
      simp [alpha]
    rw [heq, Complex.ofReal_re]
    exact div_pos (by exact_mod_cast (show 0 < d-1 by omega))
...
-- This diagnostic tests the actual RH clause; it does not assume a source criterion.
theorem source_rh_to_failure_target (h0 : sourceThetaCoefficient 0 = 1)
    (hNotRh : ¬ RiemannHypothesis) :
    ∃ d : ℕ, 1 ≤ d ∧ ¬ RealRoots (sourceJensenPolynomial d) := by
  by_contra! hAll
  apply hNotRh
  intro s hs htrivial hone
  simp_all only [RealRoots]
