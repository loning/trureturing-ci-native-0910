...
theorem laguerre_splits (p : ℝ[X]) (hp : p.Splits) (x : ℝ) : LaguerreAt p x := by
  ...
  have hg : HasDerivAt
    ...
    convert (hasDerivAt_const x (p.roots.count z : ℝ)).fun_div
      ((hasDerivAt_id x).sub_const z) hxz using 1 <;> simp
  ...
  have hn : (∑ z ∈ p.roots.toFinset, -(p.roots.count z : ℝ)/(x-z)^2) ≤ 0 := by
    ...
    intro z _
    exact div_nonpos_of_nonpos_of_nonneg (by positivity) (sq_nonneg _)
  have hfrac : (p.derivative.derivative.eval x*p.eval x -
      p.derivative.eval x*p.derivative.eval x) / p.eval x^2 ≤ 0 := by rw [hi]; exact hn
  have hnum := (div_nonpos_iff_of_pos (sq_pos_of_ne_zero hx)).mp hfrac
...
lemma p_coeff (a : ℕ → ℝ) (d k : ℕ) (hk : k ≤ d) :
  ...
  simp [P, finsetSum_coeff, coeff_C_mul_X_pow, Finset.mem_range, show k < d+1 by omega]
...
lemma q_no_nonpositive (a : ℕ → ℝ) (d : ℕ) (hd : 1 ≤ d)
  ...
  have hpv : 0 < (P a d).eval (-1/x) := by
    have hy : 0 ≤ -1/x := div_nonneg_of_nonpos_of_nonpos (by norm_num) hx
...
lemma p_map_normalized (a : ℕ → ℝ) (d : ℕ) (hd : 1 ≤ d) :
  ...
  simp [P, map_sum, Polynomial.map_mul, Polynomial.map_pow]
