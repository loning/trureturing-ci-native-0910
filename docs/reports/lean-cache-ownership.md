...
| Lean report cache | Report reuse; `make lean-report`, explicit report cache targets, and CI | Normal report production enables a UID-owned local store and optional shared Release acquisition; CI also retains its Actions report cache. Existing input validation and the delta producer control reuse. Report storage is separate from Lean build/dependency stores, donor selection, ensure, and writer ownership. See the [Lean report cache usage guide](lean-report-cache.md). |
