...
scribe-strip:
	@dotnet run --project tools/StrataLint.Cli/StrataLint.Cli.csproj --configuration Release -- strip-scribe-receipts --base "$(BASE)" $(foreach source,$(SOURCE),--source "$(source)") $(if $(filter 1,$(DRY_RUN)),--dry-run,)
decompose:
